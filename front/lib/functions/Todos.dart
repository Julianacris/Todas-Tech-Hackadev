...
class TodosProd extends StatefulWidget {
  final Function opcaoSelecionadaFuncao;
  const TodosProd({Key? key, required this.opcaoSelecionadaFuncao}) : super(key: key);
...
class TodosProdState extends State<TodosProd> {
  late Function opcaoSelecionadaFuncao;
  late List<Produto> produtos;
  List<Produto> resultadosPesquisa = [];
  ...
  void _performSearch(String query) {
    ...
    for (var produto in produtos) {
      if (produto.nome.toLowerCase().contains(query.toLowerCase())) {
        resultadosPesquisa.add(produto);
      }
    }
  ...
  Widget build(BuildContext context) {
    List<Produto> produtosExibidos =
        resultadosPesquisa.isNotEmpty ? resultadosPesquisa : produtos;
    ...
    return GridView.count(
      ...
      mainAxisSpacing: 10,
      crossAxisCount: 2,
      children: produtosExibidos.map((produto) {
        return Produto(
          opcaoSelecionadaFuncao: opcaoSelecionadaFuncao,
          valor: produto.valor,
          nome: produto.nome,
          imagem: produto.imagem,
          quantidadeEstrelas: produto.quantidadeEstrelas,
          parcelas: produto.parcelas,
          valorDaParcela: produto.valorDaParcela,
